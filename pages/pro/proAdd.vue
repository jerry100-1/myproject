<template>
				...
				<!-- <view>{{item.specName}}</view> -->
				 <view class="shopInfo" style="width:90%;margin-left:5%;" v-for="(i,index) in item.specName" :key="index">
						<view class="flexbox" >
							...
							<input class="shopNameCls viewBot2  wviewBot2" style="width:200upx"  type="number" @input="chagnge100" v-model="i.value" placeholder="" maxlength="50"/>
						</view>		
				 </view>
				    ...
				    <view class="shopInfo" style="width:90%;margin-left:5%;">
							<view class="flexbox" >
								...
								<input class="shopNameCls viewBot2  wviewBot2" style="width:200upx" @input="getPPValue"  type="number" v-model="productpriceValue" placeholder="" maxlength="50"/>
							</view>		
				     </view>
					...
					<view class="shopInfo" style="width:90%;margin-left:5%;">
							<view class="flexbox" >
								...
								<input class="shopNameCls viewBot2  wviewBot2" style="width:200upx" @input="getMPValue" type="number" v-model="marketpriceValue" placeholder="" maxlength="50"/>
							</view>		
					</view>
					...
					<view class="shopInfo" style="width:90%;margin-left:5%;">
						<view class="flexbox" >
							...
							<input class="shopNameCls viewBot2  wviewBot2" style="width:200upx"  @input="getPSValue" type="number" v-model="productstockValue" placeholder="" maxlength="50"/>
						</view>		
					</view>				 
				    ...
				    <view class="step3">
				     	<text style="font-size:30upx;margin-left:5%">{{productpicText}}</text>
							<view class="right" style="margin-left:30%;">
								...
								<image src="../../static/capital/my_add_image@2x.png" @tap="scaleImgsChoose(1)" v-if="scaleImgs.length!=5" ></image>
							</view>
							
							<view class="textInfo" style="">
								<text>上传商品图片,图片支持jpg,gif,png格式,最多只能上传5张,</text>
								<text class="color">建议使用尺寸200x200像素以上,大小不超过1M的正方形图片;</text>
							</view>
				     </view>
	...
	<!-- 			     
		...
		<view class="step3">
			...
			<!-- 新增的list列表 -->
			<view class="step3"   style="margin-top:30upx;"   v-for="(item,index) in proctListName2" :key="index">
				<view class="flexbox">
					<text class="viewBot1" style="font-size:26upx;">规格设置1</text>
				    <view style="width:80%;background:rgba(0,0,0,0.16);position:relative;left:-30upx;border-radius:20upx;
					display:flex;flex-direction:row;align-items:center;padding:10upx 10upx;min-height:300upx;">
						<image :src="scaleImgs" style="width:120upx;height:120upx;flex-shrink:0;"></image>
						<view style="flex:1;height:200upx;text-align:left;position:relative;top:0upx;color:rgba(0,0,0,0.36);left:13upx;align-items:center;">
							  <view style="height:50upx;font-size:28upx;flex:1;width:100%;" v-for="(i,index) in item.specName" :key="index">
					
								{{i.name}}--{{i.value}}
							  </view>
							  <view style="height:50upx;font-size:28upx;flex:1;width:100%;">{{item.productMoney.name}} {{productpriceValue}}</view>
							  <view style="height:50upx;font-size:28upx;flex:1;width:100%;">{{item.agoraMoney.name}} {{marketpriceValue}}</view>
							  <view style="height:50upx;font-size:28upx;flex:1;width:100%;">{{item.stock.name}}  {{productstockValue}}</view>
							  
							  <view style="width:50upx;height:50upx;border-radius:50%;background:red;position:absolute;top:-30upx;right:40upx;
							  text-align:center;color:#fff;line-height:50upx;font-size:150%;" :data-closeindex="index"  @click="closeTheShadowDiv">
								  &times;
							  </view>
						</view>
			...
			<view class="step3" style="margin-top:30upx;">
				<view class="flexbox">
					...
					<view class="inline viewBot2" @click="improveStandard">
			...
			<view class="step3 flexboxdiv">
			    ...
			    <view class="uni-list choosePayWay" style="margin-left:-40upx;">
			...
			<view class="step3" style="margin-top:10upx;">
					<view class="flexbox">
						<text class="viewBot1 newviewBot1" style="font-size:26upx;">分享雇金</text>
						<input class="shopNameCls newviewBot2 " style="width:320upx!important;margin-left:-30upx;"  type="text" v-model="sharingofEmployment" placeholder="" maxlength="50"/>%
					</view>
		...
		<view class="step4">
			...
			<view>
				...
				<view class="smallItem" >
				 ...
				 <view class="uni-list">
						<radio-group @change="radioChange">
							<label style="display:inline-block;margin-left:60upx;" class="uni-list-cell uni-list-cell-pd" v-for="(item, index) in items" :key="item.value">
								<view>
				...
				<text style="margin-left:60upx;">非包邮商品,请到管理后台上传</text>
			...
			<!-- 以下内容是在条件满足时才会显示 -->
					 ...
					 <view class="shopInfo">
							<view class="flexbox">
								...
								<input class="shopNameCls viewBot2 wviewBot2" style="width:160upx"  type="number" v-model="timerMoney" placeholder="" maxlength="50"/>
					 ...
					 <view class="shopInfo">
							<view class="flexbox">
								...
								<input class="shopNameCls viewBot2  wviewBot2" style="width:200upx"  type="number" v-model="timerSaleCount" placeholder="" maxlength="50"/>
							</view>		
					</view>	
	...
	export default{
		...
		methods:{
			...
			improveStandard:function()
			{
			   console.log("你好,你点击了完善规格按钮10000!!!");	
				...
				// uni.setStorage({
				// 	key:"cateid",
				// 	data:that.threeCate[that.threeIndex].cateid
				// });
				let that=this;
				
				// that.productpriceValue="";
				// that.productpicValue="";
				// that.marketpriceValue="";
				// that.productstockValue="";
				...
				//         console.log("拿到的cateid是"+res.data);
				//     }
				// });				
				//this.showTheShadow=true;	
				
				that.showTheModal=true;
			...
			{	
				...
				// that.newProctListName.concat(that.proctListName);
				// that.proctListName=that.newProctListName;
				// :data-productpricetext="productPrice"
				// :data-marketpricetext="marketPrice"
				// :data-productstock="productStock"
				// :data-productpic="productPic"  
				//console.log("你好,获得的imgsrc是"+e.currentTarget.dataset.imgsrc);
				// console.log("你好,获得的value值1是"+that.specName[0].value);
			...
			expressFun2(e)
			{
				...
				// that.setTimeSell=e.detail.value
				// 
		...
		//商品三级分类
			threeCateFun(e,id){
				...
				//console.log("你好,三级联动时候我获取的catename是"+e.currentTarget.dataset.catename);
				//console.log("你好,三级联动时候我获取的cateid是"+e.currentTarget.dataset.cateid);
				//拿到cateid之后,根据它请求/shopProduct/getProductCateSpec查询三级分类规格 这个接口
				
				//;
				//console.log("你好,当前的catename是"+that.threeCate[that.threeIndex].catename);
				//console.log("你好,当前的cateid是"+that.threeCate[that.threeIndex].cateid);
				// 
				// uni.setStorage({
				// 	key:"cateid",
				// 	data:that.threeCate[that.threeIndex].cateid
				...
				// 		that.proctListName2=JSON.parse(d);
				// 		console.log(that.proctListName2,"ds");
				// 		console.log("这是agoraMoney"+that.proctListName2[0].agoraMoney.name);
				// 		that.specName=that.proctListName2[0].specName;
				// 		that.productpriceText=that.proctListName2[0].productMoney.name;
				// 		that.marketpriceText=that.proctListName2[0].agoraMoney.name;
				// 		that.productstockText=that.proctListName2[0].stock.name;
				// 		that.productpicText=that.proctListName2[0].specImg.name;
				// 		// productPrice:"",//商品售价
				// 		// marketPrice=:"",//市场定价
				// 		// productStock:"",//商品库存
				// 		// productPic:"",//商品图片
			...
			save(){
				...
				that.$ajax({
					...
					data:{
						...
						jsonArraySpec:JSON.stringify(that.proctListName2).toString()||null,
						timerStartTime:that.timerStartTime ,//定时销售开始时间
						timerEndTime:that.timerEndTime ,//定时销售结束时间
			...
			if(e.proId){
				...
				this.$ajax({
					...
					success(d){
						...
						that.timerEndTime=d.timerEndTime;
						that.timerMoney=d.timerMoney;
						that.timerSaleCount=d.timerSaleCount;
						that.timerStartTime=d.timerStartTime;
